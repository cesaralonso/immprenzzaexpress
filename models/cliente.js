...
Cliente.logicRemove = (clienteId, next) => {
    if( !connection )
        return next('Connection refused');
    connection.query('UPDATE cliente SET baja = 1 WHERE idCliente = ?', [clienteId], (error, result) => {
        if ( error )
            return next({ success: false, error: error, message: 'Hubo un error al eliminar este registro' });
        else
            return next( null, { success: true, result: result, message: 'Cliente eliminado' });
    });
};
